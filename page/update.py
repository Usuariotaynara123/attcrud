...
def atualizar():
    ...
    if button_update_select or st.session_state.upda:
        ...
        item = cliente.selecionar_matricula(update_matricula)[0]
        with st.form(key='update'):
            ...
            update_age = st.number_input(label='Insira a idade', 
                                            format='%d', 
                                            value=item[2])
            update_turno = st.selectbox(label='Insira o seu Turno',
                                        index=turno.index(item[3]))
            update_periodo = st.selectbox(label='Insira o seu Periodo',
                                        index=periodo.index(item[4]))
